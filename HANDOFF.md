...
7. **git 已建立**（Phase 0 完成）：`main` 保有基线 dd7457f；空间重构在 `feature/spatial-redesign` 分支进行，每 Phase 一个 commit（`phase1: …`）。
...
## 13. 空间脚本（第 12 轮起，视觉决策的唯一真理源）

> 本节由"空间重构方案 v1"（2026-08-28，依据 `蓝图.png` 程序化解析）确立。任何视觉改动先对照本节。

### 13.1 一条光轴

光从**左上方光窗**进入，向右下衰减。全页所有明暗与投影方向服从它：受光面朝左上，投影朝右下。右侧"深处书墙"是**全页唯一暗部**（雾蓝灰、克制），纵深的唯一来源，禁止在别处再造暗部。

### 13.2 三个平面（z 轴）

| 平面 | 内容 | 纪律 |
|---|---|---|
| 远景 z=0 | `.room`：墙面横向光梯度 / 光窗与光斑 / 深处书墙 / 地板与暮色反光 | 去掉内容层也要能独立读出"一间有光的屋子" |
| 中景 z=1 | 纸感容器 / 木架板 / 书 / 索引签 | 同一套材质语言，共享光的方向 |
| 前景 z=2 | 便签 / 盆栽 / 茶杯等 | 全页 ≤5 件，逐件有空间因果，允许跨区块遮挡 |

### 13.3 竖向旅程（滚动 = 在房间里走动）

窗边（Hero，最亮）→ 桌面（继续阅读，叠压缝合）→ 书架前（我的书架，最实体）→ 墙上（分类索引 + 手记，轻）→ 地面（书签纸片 + Footer 地板暮色，收束）。

### 13.4 数值锚点（Agent 硬约束）

| 指标 | 目标 |
|---|---|
| 墙面横向亮度差（左右边缘） | 15–18%（程序化验收 ≥10%，因内容层会稀释） |
| 右侧深处书墙存在感（opacity） | 0.18–0.28 |
| 地板光斑 | 与光窗方向一致（斜落右下） |
| 地板暮色反光 | ≤0.12，几乎察觉不到 |
| 前景物总数 | ≤5，逐件登记因果 |
| 相邻区块视觉连接 | 每对 ≥1 处 |
...
*文档由上一轮实现方整理，内容与代码实际状态一致（已逐项核对文件与数值）；第 13 节由空间重构方案 v1 增补。*
