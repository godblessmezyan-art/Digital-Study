import { icons } from './icons.js?v=cloud-realm-study-12';
import {
  books, categories, dailyBookmarks, notes, popularCategories, quotes, stats,
} from './data.js?v=cloud-realm-study-13';
import { initTheme } from './theme.js?v=cloud-realm-study-12';
...
app.innerHTML = `
  ...
  <aside class="theme-popover" id="settings"><h3>书房主题</h3><p>场景、配色与内容彼此解耦，新增主题只需扩展一份配置。</p><button class="theme-option" id="atmosphere"><span class="theme-swatch"></span><span><strong>${theme.name}</strong><small>点击切换晨光 / 暮色氛围</small></span></button></aside>`;
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'Escape') document.querySelector('#settings').classList.remove('is-open');
});
