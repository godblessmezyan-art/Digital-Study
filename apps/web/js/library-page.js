...
function header(){return `<header class="lm-header"><div><span>ARCHIVE MANAGEMENT</span><h1>藏书管理</h1><p>统一管理书籍、分类与标签</p></div><nav class="lm-tabs" aria-label="藏书管理模块">${[['books','书籍'],['categories','分类'],['tags','标签']].map(([id,label])=>`<button class="${state.activeTab===id?'active':''}" data-lm-tab="${id}">${label}<small>${id==='books'?state.books.length:id==='categories'?state.categories.length:state.tags.length}</small></button>`).join('')}</nav></header>`}
